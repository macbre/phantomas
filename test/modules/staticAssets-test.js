...
vows.describe('staticAssets').
	addBatch({
		...
		'no gzip': mock.getContext('staticAssets', function(phantomas) {
			return phantomas.recv({url: URL, status: 200, isCSS: true, type: 'css'}).report();
		},
		{
			'assetsNotGzipped': 1,
		}),
		'with query string': mock.getContext('staticAssets', function(phantomas) {
			return phantomas.recv({url: URL + '?foo=bar', status: 200, isCSS: true, type: 'css'}).report();
		},
		{
			'assetsWithQueryString': 1,
		}),
		'with cookies': mock.getContext('staticAssets', function(phantomas) {
			return phantomas.recv({url: URL, status: 200, isCSS: true, type: 'css', hasCookies: true}).report();
		},
		{
			'assetsWithCookies': 1,
		}),
		'multiple requests': mock.getContext('staticAssets', function(phantomas) {
			var entry = {url: URL, status: 200, isCSS: true, type: 'css'};
		...
		'normal images': mock.getContext('staticAssets', function(phantomas) {
			return phantomas.recv({url: URL, status: 200, isImage: true, type: 'image', contentLength: 32 * 1024}).report();
		},
		{
			'smallImages': 0,
		}),
		'small images': mock.getContext('staticAssets', function(phantomas) {
			return phantomas.recv({url: URL, status: 200, isImage: true, type: 'image', contentLength: 1024}).report();
		},
		{
			'smallImages': 1,
		}),

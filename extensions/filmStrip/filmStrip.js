...
module.exports = function (phantomas) {
  if (!phantomas.getParam("film-strip")) {
    phantomas.log(
      "filmStrip: to enable screenshots of page being loaded run phantomas with --film-strip option"
    );
    return;
  }

  var filmStripOutputDir = phantomas
      .getParam("film-strip-dir", "filmstrip", "string")
      .replace(/\/+$/, ""),
  ...
  phantomas.log(
    "filmStrip: film strip will be stored as %s/%s-*.png files (zoom: %d)",
    filmStripOutputDir,
    filmStripPrefix,
    zoomFactor
  );
  ...
  if (phantomas.getParam("film-strip", false, "string")) {
    ...
    phantomas.log(
      "Film strip: screenshots will be taken at: %s ms",
      timePoints.join(", ")
    );
    ...
    ["loadStarted", "send", "recv", "loadFinished", "report"].forEach(function (
      ev
    ) {
  ...
  phantomas.on("report", function () {
    phantomas.log(
      "Film strip: spent %d ms on rendering %d screenshots",
      timeTotal,
      screenshots.length
    );

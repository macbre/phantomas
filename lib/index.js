		...
		try {
			...
			events.once('recv', async entry => {
				if (!firstResponseReceived && entry.status >= 400) {
					debug('<%s> response code is HTTP %d %s', entry.url, entry.status, entry.statusText);

					// close the browser before leaving here, otherwise subsequent instances will have problems
					await browser.close();
					reject(new Error('HTTP response code from <' + entry.url + '> is ' + entry.status));
				}
			...
			await events.emit('init', browser, page);
			...
			// resolve our run
			await events.emit('beforeClose', page);
			await browser.close();

			// your last chance to add metrics
			await events.emit('report');

...
function getModuleMetadata(moduleFile) {
	...
	if (matches) {
		var desc = matches[0].
			replace(/^[\s/]?\*+\s?/mg, ''). // remove the beginning of block comment
			replace(/\/$/mg, ''). // remove the ending of block comment
			replace(/^\s?setMetric.*$/mg, ''). // remove setMetric annotations
			trim();
...
[].concat(
	...
	// modules ...
	if (moduleFile.indexOf('/scope.js') === -1 && moduleFile.indexOf('/modules/') > -1) {

...
function getModuleMetadata(moduleFile) {
	var content = fs.readFileSync(moduleFile),
		...
		matches,
		moduleName,
		re =/(setMetric|incrMetric)\(['"]([^'"]+)['"](\)|,)(.*@desc.*$)?/mg;
	...
	while ( (matches = re.exec(content)) !== null ) {
		...
		if (hasDesc) {
			...
			// mark unreliable and optional metrics
			['unreliable', 'optional'].forEach(function(marker) {
			...
			else if (/^(number of|total number of|average specificity|total specificity)/.test(metricComment)) {
				metricUnit = 'number';
			}
			else if (/^(size|length|total length) of/.test(metricComment)) {
				metricUnit = 'bytes';
			}

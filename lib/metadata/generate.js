...
function getMetricsCoveredByTests() {
	const debug = require('debug')('metricsCovered'),
		raw = fs.readFileSync(__dirname + '/../../test/integration-spec.yaml').toString(),
...
const covered = getMetricsCoveredByTests();

debug('Found %d metrics covered by integration-spec.yaml...', covered.length);
...
[].concat(
	...
	// skip scope.js files
	if (moduleFile.indexOf('/scope.js') > -1) {
		return;
	}
	...
	Object.keys(data.metrics).forEach(function(metricName) {
		...
		metadata.metrics[metricName].testsCovered = (covered.indexOf(metricName) > -1);
	});
...
debug('Metrics found: %d (tests coverage: %s%)', metadata.metricsCount, (covered.length / metadata.metricsCount * 100).toFixed(2));

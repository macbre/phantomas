...
function getModuleMetadata(moduleFile) {
	...
	while ( (matches = re.exec(content)) !== null ) {
		...
		if (hasDesc) {
			...
			['unreliable', 'optional', 'offenders'].forEach(function(marker) {
			...
			else if (/^(number of|total number of|average specificity|total specificity|median of number|maximum number)/.test(metricComment)) {
				metricUnit = 'number';
			}
